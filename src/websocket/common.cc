...
future<> connection<is_client, text_frame>::handle_ping(temporary_buffer<char> buff) {
    return send_data(opcodes::PONG, std::move(buff));
}

template <bool is_client, bool text_frame>
future<> connection<is_client, text_frame>::handle_pong() {
    // TODO
...
template <bool is_client, bool text_frame>
future<> connection<is_client, text_frame>::send_data(opcodes opcode, temporary_buffer<char> buff) {
...
template <bool is_client, bool text_frame>
future<> connection<is_client, text_frame>::response_loop() {
...
template <bool is_client, bool text_frame>
void connection<is_client, text_frame>::shutdown_input() {
    _fd.shutdown_input();
}

template <bool is_client, bool text_frame>
future<> connection<is_client, text_frame>::close(bool send_close) {
...
template <bool is_client, bool text_frame>
future<> connection<is_client, text_frame>::read_one() {
...
template class connection<true, false>;
template class connection<true, true>;
template class connection<false, false>;
template class connection<false, true>;
