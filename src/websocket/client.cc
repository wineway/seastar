...
client_connection<text_frame>::client_connection(connected_socket&& fd, sstring resource,
                                     ...
                                     handler_t handler)
    : connection<true, text_frame>(std::move(fd))
...
future<> client_connection<text_frame>::process() {
    ...
    co_await when_all_succeed(
        this->_handler(this->_input, this->_output).handle_exception([this] (std::exception_ptr e) {
            return this->_read_buf.close().then([e = std::move(e)] () mutable {
                return make_exception_future<>(std::move(e));
            });
        }),
        do_until([this] { return this->_done; }, [this] { return this->read_one(); }),
        this->response_loop()
    ).discard_result();
}
